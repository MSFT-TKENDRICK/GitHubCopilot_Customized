...
# ── Stdlib LLM shim (no litellm / tiktoken compilation needed) ────────────

def _make_github_models_lm(model_name: str = "openai/gpt-4o"):
    """Return a LanguageModel callable backed by GitHub Models via urllib.

    Uses only Python stdlib so GEPA optimize runs without litellm, tiktoken,
    or aiohttp -- all of which require compiled extensions unavailable on
    Windows ARM64 without a full MSVC + Rust toolchain.
    """
    import json
    ...
    def _get_token() -> str:
        try:
            ...
            pass
        for env_var in ("GITHUB_TOKEN", "OPENAI_API_KEY"):
    ...
    def _lm(prompt):
        ...
        payload = json.dumps({"model": bare_model, "messages": messages}).encode()
        token = _get_token()
...
def optimize_skill(
    ...
    proposer_lm = _make_github_models_lm(model)
    ...
    result = oa.optimize_anything(
        ...
        objective=(
            ...
            f"Include clear WHEN: trigger phrases and distinguish from other skills."
        ),
        background=(
            f"This is a SKILL.md for GitHub Copilot. The LLM reads it to decide which "
            f"skill to invoke. It competes with other skills for selection. "
